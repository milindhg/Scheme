  ...
  (lambda (expr env k)
    (union-case expr exp
      ...
      [(var v) 
       ...
       (apply-env env num k))]
      [(if test conseq alt)
       (let* ([expr test] [k (kt_inner_if conseq alt env k)])
       (value-of-cps expr env k))]                               
      [(mult rand1 rand2) 
       (let* ([expr rand1] [k (kt_outer_mult rand2 env k)]) 
       (value-of-cps expr env k))]
      [(sub1 rand) 
       (let* ([expr rand] [k (kt_inner_sub1 k)]) 
       (value-of-cps expr env k))]
      [(zero rand) 
       (let* ([expr rand] [k (kt_inner_zero k)]) 
       (value-of-cps expr env k))]
      [(capture body)
       (let* ([expr body] [env (envr_extend k env)])
       (value-of-cps expr env k))]
      [(return vexp kexp)
       (let* ([expr kexp] [k (kt_inner_return vexp env)])
       (value-of-cps expr env k))]
      [(let vexp body)
       (let* ([expr vexp] [k (kt_inner_let body env k)])
       (value-of-cps expr env k))]
      [(lambda body) 
       (let* ([k^ k] [v (clos_closure body env)])
       (apply-k k^ v))]
      [(app rator rand)
       (let* ([expr rator] [k (kt_outer_rator_rand rand env k)]) 
       (value-of-cps expr env k))])))
	 ...
	 (let* ([expr alt])
      ...
      [(inner_mult x k) 
       (let* ([v (* x v)]) 
         (apply-k k v))]
      [(outer_mult rand2 env k) 
       (let* ([expr rand2] [k (kt_inner_mult v k)])
         (value-of-cps expr env k))]
      [(inner_sub1 k) 
       (let* ([v (- v 1)])
           (apply-k k v))]
      ...
      [(inner_rator_rand y k) 
       (let* ([c y] [a v])
         (apply-closure c a k))]
      [(outer_rator_rand rand env k) 
       (let* ([expr rand] [k (kt_inner_rator_rand v k)])
         (value-of-cps expr env k))])))
;;---------------------------------------------------------------------------------------------------------------------------------
;;Testcases
