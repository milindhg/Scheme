...
(define handle-union-case-case
  (lambda (name env)
    (lambda (template body)
      (pmatch-who "handle-union-case-case" template 
...
(define get-body
  (lambda (c)
    (pmatch-who "get-body" c
      [`(,test ,body) body])))

(define remove-last
  (lambda (ls)
    (pmatch-who "remove-last" ls
      [`((else ,body)) '()]
      [`((,test ,body) . ,c*) `((,test ,body) . ,(remove-last c*))])))

(define apply-env
  (lambda (env x)
    (pmatch-who "apply-env" env
...
(define parse-function-body
  (lambda (tail env)
    (if tail
        (lambda (expr)
          (pmatch-who "parse-function-body" expr
            [`(error ,name ,msg) 
             (pc2c-append
              "fprintf(stderr, \"" msg "\");\n exit(1);\n")]
            [`(if ,test ,conseq ,alt) 
        ...
        (lambda (expr)
          (pmatch-who "inner-parse-function-body" expr
	    ;; [(error ,name ,msg) 
            ;;  (pc2c-append
            ;;   "fprintf(stderr, \"" msg "\");\n exit(1);\n")]
...
(define pc2c-header-parse
  (lambda (decl)
    (pmatch-who "pc2c-header-parse" decl
      [`(load ,file . ,file*)  ""]
      [`(exit)  ""]
